# Luna and the Gray Cities

**Source**: mikel, 2026-09-12 (Kestrel Book 4 sitting; ruled while naming the Church's higher authority). Ruled canon; derived items marked.
**Scope**: system-wide.

---

## The Gray Cities

**"The Gray Cities" means Luna** — its numerous large and ancient settlements, among the oldest and most mature human settlements off Earth.

**The name is a misnomer, and an outsider's.** They are called gray for the color of the lunar surface, not for anything about the settlements themselves. The cities are **huge domed cities built into craters**, and their actual dominant color is **gold**: the environment domes carry a layer of gold on their outer surface for radiation shielding.

*Derived, for writing:* the name is what a place gets called by people who have never been — the ground's color standing in for the city's. A traveller's first sight of Luna is a plain of gray pocked with gold. The gap between the name and the thing is free irony wherever it is wanted, and it is never worth explaining on the page.

## The Church of the Makers

**Luna is the homeworld of the Church of the Makers.** Its **overall headquarters and political center is at Tycho.**

**"The Gray Cities" is used metonymically for that seat** — the way a capital city's name stands for a government. When someone off Luna says a matter has gone to the Gray Cities, they mean it has gone to Tycho, to the Church's higher authority.

## Consequences (derived — not ruled)

- **The Church's higher authority cannot be consulted in real time from anywhere but Luna.** Fabric channels are not faster than light (see the channel canon in the Kestrel Book 4 reference layer). Mars to Luna is roughly **3 to 22 light-minutes one way** depending on where the planets stand, so a question and its answer are **6 minutes to three quarters of an hour apart**. A prelate on Mars who refers a dispute to the Gray Cities is not getting an answer in the next exchange, and everybody in the room knows it.
- A Church officer on a far world therefore acts on his own judgment and is *later* ratified or not. That is an ordinary fact of the institution, not a crisis, and it shapes what a prelate's local decisions are worth.
- **Available to Kestrel Book 4 at `10/1`**, where the Prelate of Woaradan, barricaded and citing a clause, is in communication with the Gray Cities: the delay is dramatically useful and costs nothing to show.

---

**Related**: `12-earth.md`; the Church's internal politics in `08-book3-novel-scan.md`.
